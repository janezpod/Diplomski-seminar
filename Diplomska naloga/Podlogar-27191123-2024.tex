...
prirejanja. Binarno kodiranje prepisovalnih pravil dopustne gramatike $G_w$ skupaj s 
predstavljenim prirejanjem zagotavlja dobro zgornjo mejo odvečnosti. Takšno stiskanje podatkov je 
univerzalen kod.}
...
provide an example for each class. Binary encoding of the production rules of an admissable
grammar $G_w$ with the underlying admissable grammar assignment results in good redundancy
bound. Such data Compression constitutes a universal code.}
...
\begin{definicija}
    Naj bo $S \subseteq \{ 1,2, \ldots, M\}^n$. \emph{Leksikografksi indeks $S$} je preslikava
    \[
...
\begin{definicija}
    ...
    zaporedje črk $a_1, a_2, \ldots, a_n \in \A$, da je $a_1 < a_2 < \ldots < a_n$.
\end{definicija}
...
\begin{definicija}\label{def:LeksikografskoKodiranje}
    ...
    \begin{itemize}
        ...
        \item $B_2$ eniška koda vsake frekvenc $f(a_i|w)$ v enakem redu kot nastopajo črke v $B_1$;
...
\begin{primer}\label{primer:leksikografsko}
    Naj bo $\A = \{ 1, 2, 3, 4, 5 \}$. Leksikografsko kodiramo $w = 1211223$. Dobimo
    $B_1 = 11100$, saj črke $1,2,3$ nastopajo v $w$, medtem ko $4,5$ ne. Ker je
    \[
...
\begin{definicija}
    ...
    \[
        i(A) = - \log_b p(x),
    \]
    kjer je $b > 1$. V najpogostejšem primer, ko je $b=2$, merimo Shannonovo informacijo v 
    \emph{bitih}. 
\end{definicija}
...
\begin{definicija} % Discrete memoryless source
    ...
    ga dobimo tako, da staknemo $n$ realziacij diskretna slučajna spremeljivka $X$.
\end{definicija}
...
definira vir brez spomina, zato porazdelitev ocenimo. Slovenščino modeliramo z virom brez spomina
tako, da porazdelitev ocenimo preko frekvenc črk v slovenskem besedilu.

\begin{definicija}
    Imamo vir $X$ brez spomina abecede $\A$, kjer ne poznamo porazdelitve slučajne spremenljivke
    $X$ in $w$ in $w$ niz generiran z virom $X$ brez spomina abecede $\A$ dolžine $n$.
...
\begin{lema}\label{lema:ocena}
    Naj bo $w$ niz abecede $\A$ dolžine $n$. Potem velja 
    \[
...
\begin{izrek}\label{izrek:EntropijaKodiranje}
    ...
    Če niz stisknemo v manj kot $n \cdot H(X)$ bitov, skoraj gotovo pride do izgube informacij.
\end{izrek}
...
Več o odvečnost v povezavi s končnimi viri abecede $\A$ stopnje $m$,
...
ne pojavi v nobenei izpeljavi niza, ki je v jeziku KNG.
...
\begin{definicija}
    ...
    ima prazen jezik in prazen niz ne nastopa kot desni član kateregakoli prepisovalnega 
    pravila v $ P $.
\end{definicija}
...
\begin{dokaz}
    ...
    , da $A$ nastopa v vvsaj v enem izmed njih in $S \Rightarrow \alpha_1 \Rightarrow 
...
\begin{dokaz}[Dokaz izreka~\ref{izrek:ekvivalence}]
    ...
    \begin{itemize}
        ...
        KNG $G$ ne vsebuje neuporabnih simbolov, saj vsak simbol iz $V \cup \Sigma$ nastopa v vsaj enem 
        izmed nizov $f_G^i(S)$ za $i=0, 1, \ldots, \abs{V}$.
    \end{itemize}
\end{dokaz}
...
\begin{dokaz}
    ...
    \begin{itemize}
        ...
        $\{ f^k(\alpha) \mid k = 1, 2, 3 \ldots \}$ dobimo tako, da odstraimo prvi člen zaporedja
...
Nizu $w$ abecede najprej priredimo KNG $G_w$ in jo nato binarno kodiramo.
...
simbole, njihov \emph{naravni vrstni red} je določen z naraščanjem indeksa.
...
\begin{definicija}
    Z $\abs{G}$ označimo vsoto dolžin desnih članov prepisovalnih pravil KNG $G$.
\end{definicija}
...
\begin{definicija}
    ...
    \begin{itemize}
        ...
        \item Poiščemo najmanjši $i_2 \in \N$, da $u_2 = w_{i_1} \cdots w_{i_2}$ ne nastopa v 
        $\sigma_{\text{lz}}(w)$. Dodamo $u_2$ v $\sigma_{\text{lz}}(w)$. \\
        \begin{center}
        $\vdots$
        \end{center}
        \item Poiščemo najmanjši $i_{m} \in \N$, da $u_{i_m} = w_{i_{m-1}} \cdots w_{i_m}$
...
\begin{proof}
    ...
    \]
    Preprosto preverimo, da je$\abs{G^\text{lz}_w} = 3 \cdot \abs{\sigma_\text{lz}(w)}$, zato
    \[
...
\begin{primer}
    ...
    $\sigma_{\text{bis}}(w) = \{ 0001010, 0001, 01, 00, 1, 0 \}$. Prepisovalna pravila
    dopustne gramatike $G^\text{bis}_w$ so
    \begin{align*}
...
\begin{definicija}
    ...
    \]
    \emph{niz vseh desnih članov prepisovalnih pravil}. Niz $\rho_G$ brez prve pojavitve
    nekončnih simbolov $A_1, \ldots, A_{\abs{V}-1}$ označimo z $\omega_G$.
...
\begin{dokaz}
    ...
    \begin{itemize}
        ...
        \item $B_2$ je stik vseh eniških kod dolžin desnih članov prepisovalnih pravil. Sledi 
        $\abs{B_2} = \abs{G}$.
        ...
        abecednem vrstnem redu nato pa nekončne simbole v naravnem abecednem vrstnem redu. 
        Sledi $\abs{B_5} = \abs{G}$.
...
\begin{primer}
    ...
    \begin{enumerate}
        ...
        \[
            \rho_G = A_1aA_2A_3baA_4abbA_1.
        \]
        Ker poznamo dolžine desnih članov prepisovalnih pravil, iz niza $\rho_G$ dopolnimo 
...
dinamično prepozna in izkoristi ponavljajoče vzorece v sporočilu. To zagotavlja skoraj optimalne
stopnje stiskanja brez potrebe po predhodnem znanju informacijskega vira. Izkaže se, da iz 
