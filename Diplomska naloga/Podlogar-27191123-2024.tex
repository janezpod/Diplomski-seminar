...
Nizu $w$ iz abecede priredimo dopustno gramatiko $G_w$, katere jezik je $\{ w \}$.
Predstavimo dva razreda prirejanj dopustne gramatike nizu in za vsak razred podamo primer
prirejanja. Podamo binarno kodiranje prepisovalnih pravil dopustne gramatike $G_w$, 
ki skupaj s predstavljenim prirejanjem zagotavlja dobro zgornjo mejo odvečnosti. Takšno
stiskanje niza je univerzalna koda.}

% - povzetek v angleščini
\abstract{We define a subclass of context-free grammars, called admissable grammars. For each
string $w$ of an alphabet, we assign an admissable grammar $G_w$, such that its language
is $\{ w \}$. We introduce two classes of assigning an admissable grammars to a string and 
provide an example for each class. We present a binary encoding of the production rules of an
admissable grammar $G_w$, which results in good redundancy bound. Such compression constitutes a 
universal code.}
...
\begin{primer}
    ...
    \]
    Za niz $w = a_1a_2 \ldots, a_n \in \A^*_s$ kodno preslikavo $\kappa$ definiramo po črkah
    \[
...
\begin{definicija}
    ...
    \]
    Razmerju $\frac{\abs{\kappa(w)}}{\abs{w}}$ pravimo \emph{razmerje stisljivosti}. 
...
Formulo za leksikografski indeks bi lahko posplošili na poljubno \emph{linearno urejeno} abecedo.
Namesto tega za poljubno abecedo $\A$ velikosti $M$ podamo bijektivno preslikavo $\xi_s$ v 
...
$w \in \A^*$ preslikavo po črkah z $\xi_s$ v $\{ 1,2, \ldots, M\}^*$ in mu nato priredimo 
leksikografksi indeks. Podobno za inverz, najprej izračunamo inverz leksikografskega indeksa 
v abecedi $\{ 1,2, \ldots, M\}$ in ga nato po črkah z $\xi_s^{-1}$ preslikavo nazaj v niz 
abecede $\A$.
...
\begin{primer}
    Naj bo $\A = \{ a, b, c, d, e \}$ in $\xi_s(a) = 1$, $\xi_s(b) = 2$, $\xi_s(c) = 3$, 
    $\xi_s(d) = 4$, $\xi_s(e) = 5$. Leksikografsko kodirajmo niz $w = \textit{abaabbc} \in \A^*$.
    Ker niz $w$ po črkah z $\xi_s$ preslikamo v niz $1211223 \in \{ 1, 2, 3, 4, 5 \}^*$ je 
...
\begin{primer}
    ...
    Potem ocenimo
    \[
        140 = \abs{S(w)} \leq 2^{7 \cdot H(X)} \doteq 1130.
    \]
\end{primer}
...
blizu je $\kappa$ optimalnemu stiskanju. Sledeča definicija odvečnost meri kako blizu smo
...
\begin{primer}
    Formalizirajmo formalno gramatiko iz primera~\ref{primer:motivacija}. Dobili smo jo z nizom
...
Nizu $w$ abecede najprej priredimo KNG $G_w$, ki jo nato binarno kodiramo.
...
Od tu naprej, naj bo $\A$ poljubna abeceda, $\abs{\A} \geq 2$. Fiksiramo končno množico simbolov 
$\{ A_0, A_1, \ldots \}$, katere elemente bomo uporabljali za nekončne simbole.
Predpostavimo, da je $\A \cap \{ A_0, A_1, \ldots \} = \emptyset$.
Množica $\{ A_0, A_1, \ldots \}$ ima \emph{naravni abecedni vrstni red} $A_0, A_1, A_2, \ldots$.

\subsection{Prirejanje gramatike}

Namen dopustne gramatike je generirati niz sestavljen iz simbolov $\A$, zato ni pomembno, 
katere simbole se uporabllja za nekončne simbole.
...
\begin{definicija}\label{def:G(A)} 
    ...
    izreka~\ref{izrek:BinarnoKodiranje} omogočil pravilno dekodirati naslednji nekončni simbol.

    KNG $G \notin \G(\A)$, ki izpolnjuje zahtevi $1.$ in $2.$, preimenujmo nekončne simbole
    tako, da izpolnimo $5.$ zahtevo. Potem zadostimo tudi $3.$ in $4.$ točki in dobimo
    $[G] \in \G(\A)$ in velja $L([G]) = L(G)$, ki jo imenujemo \emph{kanonično oblika $G$}.
\end{definicija}
...
\begin{primer}\label{primer:kanonična}
    ...
    \[
        \mathit{SBaCDbaaCabbaabBabbaabDb}.
    \]
    ...
    \end{gather*}
    Tako dobimo $[G] \in \G(\A)$ s prepisovalnimi pravili
    \[
...
to preslikava, ki vsakemu nizu abecede $\A$ dodeli dopustno gramatiko iz $\G(B)$, ki ta niz 
generira.
...
Osredotočimo se na dva razreda prirejanj gramatike: asimptotsko kompaktno prirejanje gramatike
in neskrčljivo prirejanje gramatike. Spomnimo se endomorfizma $f_G^\infty$ iz 
definicije~\ref{def:InftyEndo}.

\begin{definicija}
    Z $\G^*(\A)$ označimo pravo podmnožico množice $\G(\A)$, tako da za vsak $G \in \G^*(\A)$ velja
    \[
...
\begin{definicija}[Lempel-Ziv prirejanje gramatike]
    ...
    \emph{Lempel-Ziv dopustno gramatiko $G^\text{lz}_w$} je
    \begin{itemize}
        ...
        \]
        \item Za $u \in \sigma_{\text{bis}}(w)$ naj bo $u = \alpha b$, kjer je $\alpha \in \Sigma^*$
        in $b \in \Sigma$. Za vsak $u \in \sigma_{\text{bis}}(w)$ imamo prepisovalno pravilo
        \[
            A_u \rightarrow A_{\alpha}b.
        \]
    \end{itemize}
    Da dobimo kanonično obliko $[G^\text{lz}_w]$ ustrezno preimenujemo nekončne simbole po
    postopku opisanem v definiciji~\ref{def:G(A)} in pokazanem na primeru~\ref{primer:kanonična}.
...
\begin{proof}
    ...
    \]
    Ker velja $\abs{G^\text{lz}_w} = 3 \abs{\sigma_\text{lz}(w)}$, sledi
    \[
...
\begin{definicija}[Bisekcijsko prirejanje gramatike]
    Naj bo $w = w_1w_2 \cdots w_n \in A^+$ in množica podnizov niza $w$ 
    \[
        \sigma_{\text{bis}}(w) = \{ w \} \cup \Bigl\{ w_iw_{i+1} \cdots w_j \Bigm| \log_2(j-i-1) \in \N_0
        \text{ in } \frac{i-1}{j-i-1} \in \N_0 \bigr\}.
    \]
    \emph{Bisekcijska dopustno gramatiko $G^\text{bis}_w$} je
    \begin{itemize}
        ...
        \]
        kjer je $u_1u_2 \cdots u_t = w$, enolično določeno tako da za vsak
...
\begin{primer}
    ...
    vsebovani. Poglejmo podnize dolžine $2$. Zaradi drugega pogoj se po nizu ``premikamo'' s
    korakom dolžin $2$. Množica $\sigma_\text{bis}(w)$ vsebuje podčrtane podnize
    \[
        \underline{00} \, \underline{01} \, \underline{01} \, 0.
    \]
    Za podnize dolžine $4$ množica $\sigma_\text{bis}(w)$ vsebuje podčrtan podnize
    \[
        \underline{0001} \, 010.
    \]
    Če podčratmo vse nize, ki so vsebovani v $\sigma_{\text{bis}}(w)$, vidimo zakaj jo imenujemo
    Bisekcijska dopustna gramatika.
    \[
    ...
    Prepisovalan pravila bisekcijske dopustne gramatike $G^\text{bis}_w$ so
    \begin{align*}
...
\begin{definicija}
    ...
    \begin{enumerate}
        ...
        desnega člana kateregali prepisovalnega pravila več kot enkrat na neprekrivajočih se mestih. 
    \end{enumerate}
\end{definicija}
...
Različne neskrčjiva prirejanja gramatike dobimo tako, da izvajamo različne sisteme ``skrčitev''.
Predstavimo nabor pravila po katerih iz poljubne dopustne gramatike v končno mnogih korakih 
...
\begin{pravilo}
    ...
    Izberemo ``svež'' $B \notin \{ A_0, A_1, \ldots \} \setminus V \cup \Sigma$. Če v prepisovalnih pravilih dopustne gramatike $G$ v enem koraku
    \begin{itemize}
...
\begin{pravilo}
    ...
    Izberemo ``svež'' $C \notin \{ A_0, A_1, \ldots \} \setminus V \cup \Sigma$. Če v prepisovalnih 
...
\begin{trditev}\label{trditev:Skrčitev}
    ...
    na $G$ uporabili neko skrčitveno pravilo. Potem je $C(G^\prime) < C(G)$. 
\end{trditev}
...
\begin{dokaz}
    ...
    \begin{enumerate}
        ...
        Sledi $C(G^\prime) < C(G)$.
        \item Ker zamenjamo vse $B$, ki nastopajo kot desni člani prepisovalnih pravil, z $A$,
        postane $B$ neuporaben simbol. Torej je $\abs{V^\prime} < \abs{V}$, saj odstranimo $B$
        in tudi $\abs{G^\prime} < \abs{G}$, saj odstranimo prepisovalno pravilo v katerem
...
\begin{definicija}[Metoda najdaljšega ujemajočega podniza]
    ...
    neprekriavjočih se mestih desnih članov prepisovalnih pravil: Uporabi skrčitveno eno
    izmed skrčitvenih pravila $2, 3, 4$. Dobimo neskrčljivo dopustno gramatiko $G^\text{nsk}_w$
    
    Da dobimo kanonično obliko ustrezno preimenujemo nekončne simbole. 
    Prirejanje $w \mapsto [G^\text{nsk}_w]$ imenujemo \emph{metoda najdaljšega ujemajočega podniza}.
\end{definicija}
...
\begin{primer}
    ...
    \begin{enumerate} 
        ...
        \end{align*}
        Uporabimo skrčitveno pravilo $2$ (lahko bi uporabili tudi skrčitveno prabilo $3$).
    ...
    Opomnimo, da je $C(G^\text{nsk}_w) = 30 < C(G_w) = 63$ in da smo do neskrčljive gramatike
    $G^\text{nsk}_w$ prišli z uporabo $6$ skrčitvenih pravil.
...
\begin{definicija}[Predelani \texttt{SEQUITUR}]
    ...
    Da dobimo kanonično obliko ustrezno preimenujemo nekončne simbole.
    Prirejanje $w \mapsto [G^\text{seq}_w]$ imenujemo \emph{predelani \texttt{SEQUITUR}} zaradi 
    njegove podobnosti z algoritmom \texttt{SEQUITUR}~\cite{NevillManningWitten1997}.
\end{definicija}
...
\begin{primer}\label{primer:EntropijaGramatike}
    ...
    \begin{gather*}
        ...
        \doteq 10.14.
    \end{gather*}
    L je niz $\omega_G$ preimenovani niza $w = 1211223$ po črkah z 
    $\xi_s(a) = 1$, $\xi_s(b) = 2$, $\xi(A_1)_s = 3$. Nizu $w$ smo v
    primeru~\ref{primer:EntropijaNiza} izračunali entropijo. Potem je
    $H(\omega_w) = H(w) \doteq 1.45$ bitov.
\end{primer}
...
\begin{dokaz}
    ...
    To, da nobena koda ni predpona druge kode, nam zagotavi enoličnost dekodiranja.
    Iz $B_1$ in $B_2$ določimo leve člane in pripadajočo dolžino desnih članov prepisovalnih pravil.
...
\begin{primer}
    Naj bo $\A = \{ a, b, c\}$, ki ima abecedni vrstni red $a < b < c$. Binarno kodirajmo dopustno 
    gramatike $G \in \G(\A)$ iz primera~\ref{primer:kanonična}. Prepisovalna pravila so
    \begin{align*}
    ...
    \begin{itemize}
        ...
        \item $B_6 = 00010100$, saj vidimo, da je niz $\omega_G$ le preimenovanje niza $w = 1211223$,
        tako kot v primeru~\ref{primer:EntropijaGramatike}. Nizu $w$ smo že v 
        primeru~\ref{primer:leksikografsko} izračunali leksikografski indeks.
    \end{itemize}
    Staknemo in dobimo
    \[
    ...
    \begin{enumerate}
        ...
        \item Ker poznamo abecedo $\A$ preberemo naslednjih $\abs{\A} = 3$ simbolov. Torej je $B_4 = 110$
        in ker poznamo abecedni vrstni red sledi $\Sigma = \{ a, b \}$.
        ...
        defkodirali binarno kodo in rekonstruirali začetno dopustno gramatiko $G$.
    \end{enumerate}
\end{primer}
...
gramatike in neskrčljivo prirejanje gramatike. V drugem razdelku pa smo spoznali binarno
kodiranje gramatike. V tem razdelku te dve preslikavi združimo v  
\emph{stiskanje niza abecede $\A$ z gramatikami $\G(\A)$} in ga posatvimo v širši kontekst stiskanja.

\begin{definicija}
    \emph{Stiskanje niza abecede $\A$ z gramatikami $\G(\A)$} je 
    par preslikav $\varPhi = (\kappa, \delta)$. Kodna preslikava je
    \begin{align*}
        \kappa \colon A^+ &\to \{ 0, 1\}^+,\\
        w &\mapsto B(\pi(w)),
    \end{align*}
    ...
    $\delta$ je dekodna preslikava preslikave $\kappa$.
\end{definicija}
...
\begin{izrek}
    ...
    ki konvergira prot $0$ tako, da je
    \[
        \max_{w \in \A^n} \frac{\abs{G_w}}{\abs{w}} = O(\nu_n).
    \]
    Potem je za vsak $m \in \N$
    \[
...
Prvi izrek pove, da odvečnost stiskanje z asimptotsko kompaktnim prirejanjem konvergira prot $0$ v 
